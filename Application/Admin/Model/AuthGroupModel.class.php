...
class AuthGroupModel extends Model {
    ...
    protected $_validate = array(
        array('title','require', '必须设置角色组标题', Model::MUST_VALIDATE ,'regex',Model::MODEL_INSERT),
        //array('title','require', '必须设置用户组标题', Model::EXISTS_VALIDATE  ,'regex',Model::MODEL_INSERT),
        array('module','require','模块必须选择'),
        array('module','checkRule','当前模块已有规则', self::MUST_VALIDATE, 'callback',Model::MODEL_UPDATE),
    ...
    public function addToGroup($uid,$gid){
        ...
        if( $del!==false ){
            foreach ($uid_arr as $u){
            	//判断用户id是否合法
            	if(M('Member')->getFieldByUid($u,'id') == false){
    ...
    static public function memberInGroup($group_id){
        ...
        $list     = M() ->field('m.id,m.username,m.last_time,m.last_ip,m.status')
                       ->table($l_table.' m')
    ...
    public function checkRule(){
        ...
        if($id && $module){
            $is = D('AuthGroup')->find($id);
            if($is['rules'] &&  $is['module']!=$module){
                return false;
            }
        }
    }
     ...
     * @author LaoHe
     * @DateTime 2017-04-05
     * @param    integer    $group_id 会员组ID
     * @param    string     $rules_id 权限ID
     * @return   bool
     */
    public function isrules($group_id, $rules_id){
        $rules=$this->where(array('id'=>$group_id))->getfield('rules');
        if(empty($rules)){
            return false;
        }else{
            $rules =explode(',',$rules);
     ...
     * @param    integer    $group_id 会员组ID
     * @param    string     $rules_id 权限ID
     * @param    integer    $type 分类 1:文章分类  2:产品分类
     * @return   bool
     */
    public function iscate($group_id, $catid, $type = self::AUTH_EXTEND_CATEGORY_TYPE){
        $map = array();
        $map['group_id'] = $group_id;
        $map['type'] = $type;
        $category = M(self::AUTH_EXTEND)->where($map)->getfield('extend_id',true);
