...
// | HICMS | 结构类生成树
// +----------------------------------------------------------------------
// | Copyright (c) 2017 http://www.zhoushan.work All rights reserved.
// +----------------------------------------------------------------------
...
class Tree{
     ...
     * @return array
     */
    public static function getParentsId($lists = [], $id = ''){
        $trees = [];
        foreach ($lists as $value) {
            if ($value[self::$config['id']] == $id) {
                $trees[] = $value[self::$config['id']];
                $trees   = array_merge(self::getParentsId($lists, $value[self::$config['pid']]), $trees);
            }
        }
        return $trees;
    }
    ...
    //传递一个分类ID返回他的子级分类
    public static function getChildCate($cate, $id) {
        $arr = array();
        $self = self::getSelf($cate, $id);
        if (empty($self)) {
            $self['id'] = 0;
