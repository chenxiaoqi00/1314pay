    jQuery(function() {
    	...
    	$('.hiUploder').each(function(index){
            ...
            uploader[index].on('beforeFileQueued',function(file){
                if(fileCount >= limit){
                    layer.msg("最多只能上传" + limit + "张图片", { icon: 5, time: 1500, shift: 6 });
                    return false;
                }
            });
            ...
            uploader[index].on("error",function (type){
                 if(type == "F_DUPLICATE"){
                    layer.msg('选择的图片有重复', { icon: 5, time: 1500, shift: 6 });
                 }else if(type == "Q_EXCEED_SIZE_LIMIT"){
                    layer.msg("所选附件不能超过5M", { icon: 5, time: 1500, shift: 6 });
                 }else if(type == "Q_TYPE_DENIED"){
                    layer.msg("文件类型错误", { icon: 5, time: 1500, shift: 6 });
                 }else if(type == "Q_EXCEED_NUM_LIMIT"){
                    layer.msg("不能超过设置的图片数量", { icon: 5, time: 1500, shift: 6 });
                 }
             });
            ...
            uploader[index] .on('ready',function(){
                $.ajax({
                    url:'/admin.php?s=/Upload/getPicture/ids/' + value + '/index=' + index,//数据库获取文件信息
                    type:'GET',
                    async:false,
