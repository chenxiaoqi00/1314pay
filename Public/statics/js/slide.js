...
$(function(){
    ...
    $root.hover(function(ev) { 
        // $root[(ev.type == 'mouseenter' ? 'add' : 'remove') + 'Class']('container-show-hover');
        // if (ie6) {
        //     pn_btn[ev.type == 'mouseenter' ? 'container-show' : 'hide']();
        // } else {
    ...
    if ($root.attr('rel') == 'autoPlay') {
       var si = setInterval(function(){
		...
		var swid = (wid-960)/2;
		var bwid = root_w * n;
		$('#container-show').css('width',wid);$('#container-show .img').css('width',wid);
		$('#container-show .btns').css('left',swid)
		$('.masks').css('width',swid);$('.mk2').css('right',0);
		$('#container-show .img span').css(({paddingLeft: swid }))
});